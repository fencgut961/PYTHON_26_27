# 🔹 Práctica 2.8 - SIEM Security Log Parser & Threat Analytics
...
Deberás estructurar tu script en Python completando de forma guiada las siguientes fases funcionales utilizando modularización de funciones con sus respectivos **Docstrings**:
...
## 🖥️ Ejemplo de Salida Esperada en Pantalla
...
🚨 Reporte Automatizado de Acciones de Firewall:
  ...
  * 192.168.1.102 -> MONITORIZACION_ESTANDAR
  * 185.220.101.5 -> BLOQUEO_INMEDIATO
  * 198.51.100.12 -> MONITORIZACION_ESTANDAR
  * 192.168.1.1 -> MONITORIZACION_ESTANDAR
  * 203.0.113.50 -> BLOQUEO_INMEDIATO
...
* **Nombre obligatorio del archivo:** Guarda la solución como **`ra2_act8_siem_parser.py`** siguiendo la convención estándar del Resultado de Aprendizaje 2 (`ra2_act<N>_<nombre>.py`).
* **Verificación de restricciones:** Asegúrate de que las Fases 1, 2 y 3 empleen comprensiones de listas explícitas, que en la Fase 4 utilices `.setdefault()` o `.get()`, y que todas las funciones incluyan su Docstring explicativo.